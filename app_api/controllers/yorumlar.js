...
const yorumGetir=function(req, res) {
  //yorumid parametresi var mi kontrol et
  if (req.params && req.params.mekanid && req.params.yorumid){
    //sadece mekan adini ve mekanin yorumlarini getir
      ...
      .exec(
        function(hata, mekan){
          ...
          if(mekan.yorumlar && mekan.yorumlar.length > 0){
            //verilen yorumid ye uygun yorum var mi?
            yorum = mekan.yorumlar.id(req.params.yorumid);
            //yoksa hata mesaji ver
            if(!yorum){
              cevapOlustur(res, 404, {'mesaj':'yorumid bulunamadi'});
            }
            //varsa cevap nesnesi döndür. içine adı, idsi ve yorumu ekle
            else{
              cevap={
                mekan:{
                  ad:mekan.ad,
    ...
    cevapOlustur(res, 404, {'mesaj':'Bulunamadi.'});
  }
};
  ...
  .exec(
    function(hata, gelenMekan){
      ...
      if (gelenMekan.yorumlar && gelenMekan.yorumlar.length > 0){
        ...
        cevapOlustur(res, 404, {'mesaj':'Guncellenecek yorum yok'});
      }
    }
  );
};
